...
class TwitterRunner:
    def __init__(self):
        ...
        self.data = self.load_tweet_data()
    ...
    def load_tweet_data(self, test_size=0.2) -> tuple[List[FeatureSet], List[FeatureSet]]:
        ...
        reviews = [
            FeatureSet.build(twitter_samples.raw(fileid), known_clas='pos' if fileid in positive_fileids else 'neg') for
    ...
    def print_results_to_file(self, results: List[tuple], accuracy: float, top_features: List[str],
                              output_file: str = "tweet_runner_results.txt"):
